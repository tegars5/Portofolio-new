// FILE: components/ToolsSection.tsx

import React from "react";
// Import IconCode dan IconBriefcase yang baru digunakan
import { IconVSCode, IconFigma, IconGit, IconTerminal, IconChrome, IconDatabase, IconCode, IconBriefcase } from "./Icons";
import { Tool } from "../types";

const TOOLS: Tool[] = [
  {
    name: "Laravel & CodeIgniter",
    description: "Backend Web Frameworks (PHP, JavaScript)",
    icon: IconBriefcase,
  },
  {
    name: "Flutter & Vue.js",
  ...
  },
  {
    ...
    description: "Utility-First and Component Styling Frameworks",
    icon: IconFigma,
  },
  {
  ...
  },
  {
    ...
    description: "Core Programming Languages (HTML/CSS included)",
    icon: IconTerminal,
  },
];
    ...
    <section id="tools" className="py-24 relative border-t border-border/50">
      <div className="max-w-5xl mx-auto px-6">
        <div className="flex items-center gap-3 mb-12">
          ...
          <h2 className="text-2xl font-semibold text-text-main">Core Technology Stack</h2>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {TOOLS.map((tool) => (
            ...
            >
              ...
              <div>
                <div className="text-sm font-medium text-text-main group-hover:text-white transition-colors">{tool.name}</div>
                <div className="text-xs text-text-muted font-normal mt-0.5">{tool.description}</div>
              </div>
            </div>
          ))}
        </div>
